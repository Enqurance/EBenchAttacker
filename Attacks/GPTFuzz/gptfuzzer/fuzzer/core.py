...
class GPTFuzzer:
    def __init__(self,
        ...
        if result_file is None:
            result_file = self.target_name + '.csv'

        file_dir = "./result/GPTFuzz/" + self.time_str + "/"
        os.makedirs(os.path.dirname(file_dir), exist_ok=True)
        
        file_path = file_dir + result_file
        ...
        self.raw_fp = open(file_path, mode, buffering=1)
        self.writter = csv.writer(self.raw_fp)
        
        if mode == 'w':
            self.writter.writerow(
                ['index', 'prompt', 'response', 'parent', 'results', 'queries'])
        ...
        for prompt_node in prompt_nodes:
            if prompt_node.num_jailbreak > 0:
                ...
                self.writter.writerow([prompt_node.index, prompt_node.prompt,
                                       prompt_node.response, prompt_node.parent.index, 
                                       prompt_node.results, self.current_query + 1])
