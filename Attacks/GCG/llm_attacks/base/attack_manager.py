...
def get_embedding_layer(model):
    ...
    elif str(type(model)) == "<class 'transformers_modules.Baichuan2-7B-Chat.modeling_baichuan.BaichuanForCausalLM'>":
        return model.base_model.embed_tokens
    elif str(type(model)) == "<class 'transformers.models.gemma.modeling_gemma.GemmaForCausalLM'>":
        return model.base_model.embed_tokens
    elif str(type(model)) == "<class 'transformers_modules.chatglm3-6b.modeling_chatglm.ChatGLMForConditionalGeneration'>":
        print(model.transformer.embedding.word_embeddings)
...
def get_embedding_matrix(model):
    if isinstance(model, GPTJForCausalLM) or isinstance(model, GPT2LMHeadModel):
        ...
        return model.transformer.embedding.word_embeddings.weight
    else:
        raise ValueError(f"Unknown model type: {type(model)}")
...
def get_embeddings(model, input_ids):
    if isinstance(model, GPTJForCausalLM) or isinstance(model, GPT2LMHeadModel):
        ...
        return model.transformer.embedding.word_embeddings(input_ids)
    else:
        raise ValueError(f"Unknown model type: {type(model)}")
    ...
    def __init__(self, 
        ...
        **kwargs,
        ...
        test_workers : list, optional
            The list of test workers used in the attack
        """
        self.goals = goals          # 攻击所用的有害问题
        self.targets = targets      # 象征攻击成功的目标，GCG不断优化后缀，令模型输出靠近攻击目标
        self.workers = workers      # 攻击使用的模型
        self.test_goals = test_goals
        self.test_targets = test_targets
        self.test_workers = test_workers
        self.control = control_init
