...
def get_args():
    ...
    parser.add_argument("--init_prompt_path", type=str, default="./Attacks/AutoDAN/assets/autodan_initial_prompt.txt")
    parser.add_argument("--dataset_path", type=str, default= "./data/EBench_test.csv")
...
if __name__ == '__main__':
    ...
    dataset = zip(harmful_data.English[args.start:], harmful_data.Goal[args.start:])
    infos = {}

    crit = nn.CrossEntropyLoss(reduction='mean')

    prefix_string_init = None
    for i, (g, t) in tqdm(enumerate(dataset), total=len(harmful_data.English[args.start:])):
        ...
        info["question"] = info["question"].join(g)
        info["goal"] = info["goal"].join(t)
        ...
        for j in range(num_steps):
            with torch.no_grad():
                ...
                print(
                    "################################\n"
                    f"Current Data: {i}/{len(harmful_data.English[args.start:])}\n"
        ...
        if not os.path.exists('./results/AutoDAN/' + time_str):
            os.makedirs('./results/AutoDAN/' + time_str)
        with open('./results/AutoDAN/' +  time_str + "/" + f'{args.model}_AutoDAN.json', 'w') as json_file:
